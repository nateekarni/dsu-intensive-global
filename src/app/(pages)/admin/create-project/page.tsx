...
export default function CreateProjectPage() {
  ...
  // Removed old classroomCount and studyPlanOptionsText state

  useEffect(() => {
    // Init with 'all' if empty
    if (allowedClassrooms.size === 0) {
      handleGradeScopeChange("all");
    }
  }, []);
  ...
  const handleGradeScopeChange = (scope: "all" | "lower" | "upper" | "gifted") => {
    ...
    if (scope === "all") {
      ...
      // Gifted logic from user requirement:
      // M.1/3, M.1/4, M.2/3, M.2/4, M.3/3, M.3/4
      // Assuming "Gifted" usually implies Lower Secondary Gifted in this context based on requirement description
      // User said: "เฉพาะห้องเรียนพิเศษ จะเป็นห้องเรียน ม.ต้น ห้อง 3 และ 4"
    ...
    try {
      ...
      await addDoc(collection(db, "projects"), {
        ...
        formConfig: {
          ...
          consents: [],
          gradeScope,
          allowedClassrooms: Array.from(allowedClassrooms), // Save as array
        },
